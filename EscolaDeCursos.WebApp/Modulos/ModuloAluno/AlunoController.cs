...
public class AlunoController(ServicoAluno servicoAluno, IMapper mapeador) : Controller
{
   ...
   public ActionResult Listar(string pesquisa)
    {
        List<ListarAlunoDto> dtos;

        if(string.IsNullOrWhiteSpace(pesquisa))
            dtos = servicoAluno.SelecionarTodos();
        else
            dtos = servicoAluno.PesquisarPorNome(pesquisa);
    ...
    {
        CadastrarAlunoViewModel cadastrarVm =
            new CadastrarAlunoViewModel(
            string.Empty,
            string.Empty,
            string.Empty
        );
